...
.controller('LibraryController', ['$scope', function($scope){
  ...
  if(localStorage.length > 2) {
    ...
    for (var i = 0; i < localStorage.length; i++) {
      //issue with also storing username in local storage
      if(isNaN(localStorage.key(i))){
        ...
        //need to create new book object to access book prototype functions
        var newBook = new Book(book.title, book.shelf, book.reference, book.presence);
        newBook.checkedOut = book.checkedOut;

        if(book.shelf.toLowerCase() === 'literature') {

          literature.push(newBook);
        }
        else if(book.shelf.toLowerCase() === 'science') {
          science.push(newBook);
        }
        else if(book.shelf.toLowerCase() === 'sports') {
          sports.push(newBook);
        }
        else if(book.shelf.toLowerCase() === 'art') {
          art.push(newBook);
        }
    }
  }

  //check to see if library had been initialized if not set or not true  add akk books to local storage
  ...
  function updateBook(bookToUpdate){
    ...
    for (var i = 1; i <= localStorage.length; i++) {
      ...
      if(book.title == bookToUpdate.title && book.shelf == bookToUpdate.shelf){
        book.presence = bookToUpdate.presence;
        book.checkedOut = bookToUpdate.checkedOut;
        localStorage.setItem(localStorage.key(i), JSON.stringify(book));
        break;
      }
